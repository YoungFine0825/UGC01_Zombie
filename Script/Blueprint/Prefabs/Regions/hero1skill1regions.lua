---@class hero1skill1regions_C:AActor
...
local hero1skill1regions = {
    ...
    -- PEBuffClassPath = "Asset/Blueprint/PESkill/PoisonGasGrenade/hero1skill1regions.hero1skill1regions_C",
...
function hero1skill1regions:ReceiveBeginPlay()
    hero1skill1regions.SuperClass.ReceiveBeginPlay(self)
   
    print("hero1skill1regions Log:ReceiveBeginPlay()")
    ...
    if UGCActorComponentUtility.HasAuthority(self) then
        ...
        function ()
            print("hero1skill1regions Log: In [TimerTriggerFunc]")
            ...
            for k, OverlappingActor in pairs(OverlappingActors) do
                if UGCObjectUtility.IsObjectValid(self:GetInstigator()) then
                    print("hero1skill1regions Log: --Character:"..tostring(self:GetInstigator()))
                    if OverlappingActor ~= self:GetInstigator() then
                        print("hero1skill1regions: Log --DamageValue:"..tostring(self.DamageValue))
                        ...
                        print("hero1skill1regions: Log --Damage:"..tostring(Damage))
                    end
                else
                ...
                for k, v in pairs(OutHits) do
                    if v.Actor:IsValid() then
                        ...
                        if UGCObjectUtility.IsObjectValid(UGCAttributeSystem.GetInstigatorFromContext(self)) then
                            print("hero1skill1regions Log: --Character:"..tostring(UGCAttributeSystem.GetInstigatorFromContext(self)))
                            if OverlappingActor ~= UGCAttributeSystem.GetInstigatorFromContext(self) then
                                ...
                                if CampRelation == ECampRelation.Same then
                                    --同阵营
                                    print("hero1skill1regions: Log -- 同阵营伤害关闭")
                                else
                                    print("hero1skill1regions: Log --DamageValue:"..tostring(self.DamageValue))
...
-- function hero1skill1regions:ReceiveTick(DeltaTime)
--     hero1skill1regions.SuperClass.ReceiveTick(self, DeltaTime)
-- end



function hero1skill1regions:ReceiveEndPlay()
    hero1skill1regions.SuperClass.ReceiveEndPlay(self) 
    UGCTimerUtility.RemoveUETimer(self.TimeHandle);
end


--[[
function hero1skill1regions:GetReplicatedProperties()
    return
end
--]]

--[[
function hero1skill1regions:GetAvailableServerRPCs()
    return
end
--]]


return hero1skill1regions