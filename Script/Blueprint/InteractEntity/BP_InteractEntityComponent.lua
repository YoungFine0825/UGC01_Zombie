...
function BP_InteractEntityComponent:ReceiveBeginPlay()
    ...
    --
    GameplaySystem.EventSystem:Listen(GameplayEvents.Server.OnPlayerInteractionCompleted, self, self.OnPlayerInteractionCompleted)
...
function BP_InteractEntityComponent:OnBeginPlayOnServer()
    ...
    --UnrealNetwork.CallUnrealRPC_Multicast(self,"RPC_Multicast_AllocInstanceID",instanceID)
end
...
function BP_InteractEntityComponent:OnRep_m_instanceID()
    --客户端像交互系统注册自身
    GameplaySystem.InteractEntitySystem:ClientRegisterEntity(self.m_instanceID,UGCActorComponentUtility.GetOwner(self),self)
    GameplayUtils.Print("BP_InteractEntityComponent.OnRep_m_instanceID: 可交互实体 ",GameplayUtils.GetUEObjClassName(self)," 获得实例ID ",self.m_instanceID)
end
...
function BP_InteractEntityComponent:OnComponentBeginOverlap(OverlappedComp, OtherActor, OtherComp, OtherBodyIndex, bFromSweep, SweepResult)
    if self.m_instanceID <= 0 then
        GameplayUtils.Exception("BP_InteractEntityComponent.OnComponentBeginOverlap: ",UGCObjectUtility.GetObjectName(self),"还未拿到唯一ID")
    ...
    if not PlayerState then
        GameplayUtils.Print("BP_InteractEntityComponent.OnComponentBeginOverlap: ",UGCObjectUtility.GetObjectName(self),"return: 重叠对象不是玩家, OtherActor=",UGCObjectUtility.GetObjectName(OtherActor))
        return--不是玩家不管
    end
    local playerKey = UGCGameSystem.GetPlayerKeyByPlayerState(PlayerState)
    if self.m_overlappedPlayerKeys[playerKey] then
        GameplayUtils.Print("BP_InteractEntityComponent.OnComponentBeginOverlap: ",UGCObjectUtility.GetObjectName(self),"return: 玩家",playerKey,"已经进入，不可重复进入")
        return--已经进入，不可重复进入
    end
    ...
    if not self.m_interactable then
        GameplayUtils.Print("BP_InteractEntityComponent.OnComponentBeginOverlap: ",UGCObjectUtility.GetObjectName(self),"return: 当前不可交互, playerKey=",playerKey)
        return--不可以交互
    end
    --
    if not self.m_isServer then--客户端逻辑还要额外检测逻辑
        ...
        if not self:CanPlayerInteract(playerKey) then--该玩家不能继续交互了
            GameplayUtils.Print("BP_InteractEntityComponent.OnComponentBeginOverlap: ",UGCObjectUtility.GetObjectName(self),"return: 玩家",playerKey,"不能继续交互（次数或CD）")
            return
        end
        --判断实体是否还能继续交互
        if not self:CanEntityInteract() then--实体无法继续交互
            GameplayUtils.Print("BP_InteractEntityComponent.OnComponentBeginOverlap: ",UGCObjectUtility.GetObjectName(self),"return: 实体总交互次数已耗尽")
            return
        end
...
---@public 增加指定玩家的交互成功次数
---@param playerKey number
---@param delta number 增加量，默认 1
function BP_InteractEntityComponent:AddPlayerInteractedTimes(playerKey, delta)
    local add = delta or 1
    self.m_playerInteractedTimes[playerKey] = (self.m_playerInteractedTimes[playerKey] or 0) + add
end

---@public 设置指定玩家最近一次交互成功时间
---@param playerKey number
---@param time number 服务端时间戳（秒）
function BP_InteractEntityComponent:SetPlayerLastInteractTime(playerKey, time)
    self.m_playerInteractedTime[playerKey] = time or 0
end
...
function BP_InteractEntityComponent:OnPlayerInteractionCompleted(playerKey, entityInstanceID, errCode,errMessage)
    if entityInstanceID ~= self.m_instanceID then
        return
    end
    if errCode == EInteractEntityErrCode.None then--交互成功
        --增加玩家交互次数
        self:AddPlayerInteractedTimes(playerKey)
        --设置玩家最佳一次交互时间
        self:SetPlayerLastInteractTime(playerKey, UGCGameSystem.GetServerTimeSec())
        --
        ...
        if not canInteract then--实体无法继续交互
            self:SetInteractableInternal(false)
        end
    end
    --服务端广播交互完成的全局事件
...
---@private
function BP_InteractEntityComponent:SetInteractableInternal(canInteract)
