...
function ShopV2_ItemGet_UIBP:Destruct()
    self.ItemList.OnUpdateItem:Remove(self.OnUpdateItem, self);
...
function ShopV2_ItemGet_UIBP:Popup(ItemID, Num)
    ...
    self.ItemData = ShopV2Manager:GetItemData(ItemID);
    self.ItemList:Reload(1);

    if self.DX_GXHD then
